...
import { popularMovies, upcomingMovies,
   nowPlayingMovies, baseImagePath } 
   from '../api/apicalls';
...
try{
let response = await fetch(nowPlayingMovies);   //this is the api name of nowPlayingMovies
let json = await response.json(); //a function that converts the data to json format
return json;
}
catch(error){
...
const getUpcominggMoviesList = async () =>{
  try{
  ...
  console.error("Something went wrong in getUpcominggMoviesList Function",error);
  }
}
...
setNowPlayingMoviesList([
  {id:'dummy1'},
  ...tempNowPlaying.results,   //results is object of nowPlayingMovies api
  {id:'dummy2'}
]);  

let tempPopular = await getPopularMoviesList();
setPopularMoviesList(tempPopular.results); //results is object of popularMovies api

let tempUpcoming = await getUpcominggMoviesList();
setUpcomingMoviesList(tempUpcoming.results); //results is object of upcomingMovies api
  ...
  <FlatList 
   ...
   renderItem={({item,index}) => {
    ...
    return(
      ...
      cardWidth={width * 0.67}
      isFirst = {index == 0 ? true:false}
      isLast = {index == nowPlayingMoviesList?.length - 1 ? true : false}
      title={item.original_title} 
      imagePath={baseImagePath('w780',item.poster_path)} //title and imagePath are the names of the props assigned at subMovieCard
...
const styles = StyleSheet.create({
  ...
  loadingTxt: {
    ...
    scrollViewContainer:{
      flexGrow: 1, //e bera flexGrow nga flex sepse me flex nuk funksiononte scrolli
      //
